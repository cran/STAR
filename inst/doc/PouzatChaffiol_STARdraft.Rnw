...
\item Using ``statistical smoothing'' techniques based on \Smoothy~\cite{Wahba_1990,Gu_2002,Gu_2008} for the plots.
\end{itemize}
...
\R\footnote{\url{http://www.r-project.org}}~\cite{R-2.7.2}
...
\item It is very easily extended by its users and, so to speak,
  encourages its users to become its programmers~\cite{Chambers_2000,Chambers_2008}.
\end{itemize}
...
holder, with its head fixed with dental wax (as shown on Fig. 2
of~\cite{EscolaEtAl_2008}). The lower part of both
...
\texttt{e070528} (the names are built with 2 digits for the year, two
...
\R~\cite{R-2.7.2}, some of its user add-on packages and two additional
...
\IPproc) is extremely useful to describe mean responses of a single
neuron to repeated presentations of a given stimulus.  
% following~\cite[Brown et al., 2002]{BrownEtAl_2002}
...
constrained to be positive\cite{BatesWatts_1988,Monahan_2001}, like the
...
build a \TQQplot s (Q-Q plots)~\cite[Chambers et al,
...
\PSTH~\cite{GersteinKiang_1960}. We discuss next these \GSSPSTH s.
...
response, that is, it's \emph{average instantaneous firing rate}. For then if the successive responses of the neuron are
...
The \GSSPSTH~of the present paper produces an estimate, $\hat{\nu}(t)$, of $\nu (t)$
through a compromise between two antagonistic goals: goodness of fit
\emph{and}~smoothness (alternative, Bayesian, methods are available to built
\gsspsth s~\cite{KassEtAl_2003,WallstromEtAl_2008}). If
...
lobe~\cite{Kaissling_1987}. Each one of these stages would smear out
...
\Smoothy~results~\cite{Wahba_1990,Gu_2002} we are going to enforce our
...
a large value will lead to a linear function (see also
Fig. 4.1-4.3~\cite[pp 46-47]{Wahba_1990} and Fig. 1.1~\cite[p 3]{Gu_2002}).
...
\begin{figure}
  ...
  \caption{Influence of \smp~choice. Comparison of fitted \GSSPSTH s
    ...
    bin width of 25 ms) appear as black and the fitted curve appears
...
Grace Wahba and her collaborators~\cite{Wahba_1990,Gu_2002}, extremely well developed. This is
...
It can be shown~\cite{KimeldorfWahba_1971},~\cite[Chap. 1]{Wahba_1990},~\cite[Chap. 2 and
5]{Gu_2002} that the function $\hat{\eta}_{\lambda}$ minimizing
...
that~\cite[Chap. 3 and 5]{Gu_2002} as detailed in
...
sample points~\cite[p 152]{Gu_2002}. Notice that we do not assume here
...
setting~\cite{CravenWahba_1979,Wahba_1990,Gu_2002}. In the present
setting, strong arguments together with simulations suggests that the
same holds but no formal proof is available~\cite{Gu_1992b}. Monte-Carlo
...
setting~\cite{XiangWahba_1996,GuXiang_2001}. These schemes do moreover
...
obtained~\cite{Wahba_1983,Nychka_1988,Wahba_1990,Gu_2002}. That is, a
...
Gaussian
regression~\cite{Wahba_1983,Nychka_1988,CumminsEtAl_2001}. Using Monte
...
\emph{general smoothing spline})~\cite{Gu_2002,Gu_2008}. The
...
\begin{enumerate}
\item Get a \gsspsth, $\hat{\nu}_{\lambda}$,~using bin width of 25 ms
...
\item For $j \, = 1 : R$ ($R$ equals 999 or 1000) do:
  \begin{enumerate}
  \item Simulate with the \emph{thinning method}~\cite[pp
...
Tierney, Rossini, Li and
Sevcikova~\cite{TierneyEtAl_,RossiniEtAl_2003}. An additional element
...
independent streams of random numbers. Pierre L'\'Ecuyer and
collaborators~\cite{LEcuyerEtAl_2002} developed such a generator which
has been made available as an \R~user contributed package by Leydold
and L'\'Ecuyer~\cite{LEcuyerLeydold_2005,Leydold_2007}. 
...
the browser. These tags are surrounded by ``<>'' symbols~\cite[Chap. 3]{Murrell_2008}. This means
...
Eric Lecoutre's package \RHTML~\cite{Lecoutre_2003}, one can write on
HTML file without having to really learn HTML. Lecoutre short 2003
paper~\cite{Lecoutre_2003} is enough to get one going within 15
minutes. 
...
8.04.1). \R~version 2.7.2 was compiled with
...
were: a dual core CPU (Intel Core 2) at 2 GHz with 1 GB of RAM.
...
paradigm of~\cite[Gentleman and Temple-Lang, 2004]{GentlemanTempleLang_2004}. This means that in addition to giving
access to the software (\STAR) and to the data sets used in the
present paper we provide a
\texttt{compendium}~\cite{GentlemanTempleLang_2004} which is
...
The user contributed package \texttt{cacheSweave} of Roger
Peng~\cite{Peng_2007} has been a tremendous help for developing our
compendium.
...
s. The excess of spikes of neuron 2 roughly 100 ms before a spike in
neuron 4 seems to be significant. 
...
intervals~\cite{Wahba_1983}~also took pain insisting that these
intervals must be interpreted ``across the function''~\cite[Wahba,
1990, p 69]{Wahba_1990}, that is, they are valid on \emph{average}
...
processes~\cite{Ogata_1988} when the \gsspsth~is taken has a model of
...
and fast (Table~\ref{spontStats} and ~\ref{responseStats}). But we insist again on the preliminary aspect
of this automatic analysis. Most of the data we showed require more
sophisticated analysis techniques.

The use of the \Smoothy~methodology has been decisive in the
...
the bias of the estimated \gsspsth. We could also consider implementing
the method described by~\cite[Cummins et al, 2001]{CumminsEtAl_2001}
to set locally the \smp~(and get a uniform coverage probability).
We have exposed here only ``the tip of the iceberg'' of what can be
...
from the AFM/Decrypton project. A. Chaffiol was supported by a DGA (?) fellowship.
...
language~\cite{IhakaGentleman_1996} \emph{does not}~make any
...
by~\cite[Gu, pp 62, 76 and 151]{Gu_2002}. 
% There is just one tiny
% mistake in the second paragraph of Sec. 3.4.1~\cite[p 76]{Gu_2002}, where
